...
export function BalancesHistoryModal({ onClose }: BalancesHistoryModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[60] animate-fadeIn p-2 sm:p-6" onMouseDown={onClose}>
      <div className={`border rounded-[2.5rem] w-full max-w-4xl max-h-[95vh] flex flex-col shadow-2xl relative overflow-hidden animate-scaleIn ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`} onMouseDown={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className={`flex items-center justify-between p-6 sm:p-8 border-b shrink-0 ${isDark ? 'border-slate-800/50 bg-slate-900/50' : 'border-slate-100 bg-slate-50'}`}>
          <h2 className={`text-xl sm:text-2xl font-bold flex items-center gap-3 ${isDark ? 'text-white' : 'text-slate-900'}`}>
            <div className={`p-2.5 sm:p-3 rounded-2xl ${isDark ? 'bg-indigo-500/20' : 'bg-indigo-100'}`}>
        ...
        {/* Content */}
        <div className="p-4 sm:p-8 overflow-y-auto flex-1 min-h-[300px] flex flex-col">
          {isLoading ? (
            ...
            <div className="space-y-4">
              {sessions.map((session) => (
                <div key={session.id} className={`border rounded-[2rem] p-6 flex flex-col md:flex-row gap-6 justify-between transition-colors ${isDark ? 'bg-slate-950/50 border-slate-800/50 hover:border-slate-700' : 'bg-slate-50 border-slate-200 hover:border-slate-300'}`}>
                  ...
                  <div className={`flex flex-wrap gap-4 sm:gap-8 items-center text-left p-4 rounded-[1.5rem] ${isDark ? 'bg-slate-900/50' : 'bg-white border border-slate-200'}`} dir="ltr">
                    <div>
                      <p className={`text-[10px] sm:text-sm mb-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{t('sessionHistory.openingBalance')}</p>
                      <p className={`font-mono text-base sm:text-xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{formatPrice(session.openingBalance)}</p>
                    </div>
                    {session.closingBalance !== undefined && (
                      <div>
                        <p className={`text-[10px] sm:text-sm mb-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{t('sessionHistory.actualBalance')}</p>
                        <p className="font-mono text-base sm:text-xl text-indigo-500 font-bold">{formatPrice(session.closingBalance)}</p>
                      </div>
                    )}
                    {session.difference !== undefined && (
                      <div className={`p-3 sm:p-4 rounded-xl border ${session.difference === 0 ? 'bg-emerald-500/10 border-emerald-500/20' : session.difference > 0 ? 'bg-indigo-500/10 border-indigo-500/20' : 'bg-red-500/10 border-red-500/20'}`}>
                        <p className={`text-[10px] sm:text-sm mb-1 ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>{t('sessionHistory.difference')}</p>
                        <p className={`font-mono text-lg sm:text-2xl font-bold ${session.difference === 0 ? 'text-emerald-500' : session.difference > 0 ? 'text-indigo-500' : 'text-red-500'}`}>
